...
vec3 getOffset(vec3 p) {
	...
	vec3 temppos = rotation3dY(time * (0.5 + 0.5 * twist_scale) + length(position.xz))*p;
	vec3 offset = fbm_vec3(position, 0.5, 0.);
...
void main () {
	...
	vec3 finalpos = position * 0.1;

	float particle_size = cnoise(position * 5.) * 0.5 + .5;

	//rotate particles
	vec3 world_pos = rotation3dY(time * 0.3 * (0.5 + 1.5 * particle_size) ) * position ;
	world_pos -= rotation3dX(time * 0.3 * (0.5 + 1.5 * particle_size) ) * position;
	...
	//vec4 view_pos = modelMatrix * vec4(position, 1.);

	vec3 particle_position = (modelMatrix * vec4(world_pos + offset + offset0, 1.)).xyz;

	vec3 dir = particle_position ;

	particle_position = mix(particle_position, normalize(dir) * 0.2, 1.);
	//particle_position.y += distanceToMouse * .2;
	...
	view_pos.xyz += position * 5. * (0.01 + 0.1 * particle_size) ;
